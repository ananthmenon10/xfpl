...
func newEntryPromotedCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "managerId"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("managerId is required\nUsage: %s <%s>", cmd.CommandPath(), "managerId"))
			}
			path = replacePathParam(path, "managerId", args[0])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "entry", false, path, params, nil)
			if err != nil {
