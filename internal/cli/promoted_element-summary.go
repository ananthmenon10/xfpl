...
func newElementSummaryPromotedCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "elementId"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("elementId is required\nUsage: %s <%s>", cmd.CommandPath(), "elementId"))
			}
			path = replacePathParam(path, "elementId", args[0])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "element-summary", false, path, params, nil)
			if err != nil {
