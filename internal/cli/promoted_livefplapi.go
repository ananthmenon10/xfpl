...
func newLivefplapiPromotedCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "https://www.livefpl.net/livefplapi/{teamId}"
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "teamId"),
					}, flags); printErr != nil {
						return printErr
					}
				}
			...
			data, prov, err := resolveRead(cmd.Context(), c, flags, "livefplapi", false, path, params, nil)
			if err != nil {
