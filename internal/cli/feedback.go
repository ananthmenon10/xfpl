...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LIVEFPL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LIVEFPL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "livefpl/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.livefpl/feedback.jsonl.
When ` + "`LIVEFPL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LIVEFPL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  livefpl feedback list
  livefpl feedback list --limit 5
  livefpl feedback list --json`,
